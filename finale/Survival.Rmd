...
```{r cars}
data<-read.csv("data/new_data/survival.csv")
library(survival)
library(survminer)
```
...
hazard_ratio
```
HR= zard_ratio=0.4250286
the risk of deaths in cluster1 (states where the preciptation are higher) is 0.4250286 times the risk in cluster2 (states where the preciptation are much lower)
