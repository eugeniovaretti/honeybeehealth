...
library(roahd)
```
This notebook will cluster the timeseries associated to the US wrt to Varroa and Minimum Temperature in order to test if they are factors for colony_lost.
...
We then cluster by Temp Min, following the same approach.
...
num_clust = read.csv("data_output/numclust.csv", header = FALSE)
names(num_clust)="n"

partitions = read.csv("data_output/clustering.csv", header = FALSE)

M <- 0.5
a <- 1900
label <- F
ob6 = plot_clustering(partitions, coord[,-3], "VI" ,M, a, ts_varroa[,-1], label = label)
ob6$plot
```
...
```{r}
plot(fd_varroa, col=as.numeric(cluster_alloc), main="Varroa trend in the different clusters") #aggungere in base a cluster
#plot(fd_tempmin, col=as.numeric(cluster_alloc))
#f_data <- append_fData(,2)
```
...
lines(grid, median4$value, col=5, lwd = 2)
lines(grid, median4$value, col=6, lwd = 2)
lines(grid, median4$value, col=7, lwd = 2)
