...
#' (anno - mese - stato) per gli US con attributi (temperatura, pioggia, 
...
read_rawstatedata <- function(ita_convert = T) {
  ...
  for (j in 1:number) 
  {
    ...
    for(i in 1:col)
    {
      ...
      if(i==1)
        parzial_result <- cbind(parzial_result,a[1:90,])
      else
        parzial_result <- cbind(parzial_result, a[1:90,3])
    }
