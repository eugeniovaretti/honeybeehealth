...
```{r}
lm_mod <- lmer(colony_lost_pct ~ months + Varroa.mites +Other.pests.parasites+Disesases+Pesticides+Other+Unknown + (1|state),
                  data = data)
summary(lm_mod)
AIC(lm_mod)
```
...
```{r}
glm_mod <- glmer(colony_lost_pct ~ months + Varroa.mites +Other.pests.parasites+Disesases+Pesticides+Other+Unknown + (1|state),
...
```{r}
library(mgcv)
mod_gam = gam(colony_lost_pct ~ months + Varroa.mites +Other.pests.parasites+Disesases+Pesticides+Other+Unknown + state, data = data)
summary(mod_gam)
```

```{r}
glmer(colony_lost_pct ~ months + Varroa.mites +Other.pests.parasites+Disesases+Pesticides+Other+Unknown + (1|state),
                  data = data,family=binomial(link="logit"))
```
