...
gam_ssplines_inter <- gam(colony_lost_pct ~ factor(year) + months + s(colony_max, bs='cr') + 
                      s(I(lon * lat), bs = 'cr') + s(Varroa.mites, bs="cr") + s(Disesases, bs="cr")
                          + s(Pesticides, bs="cr") + + s(Unknown, bs="cr") + s(Other, bs="cr"), data=df_noNa) 

#convertire months e years a un unico vettore con t=1,....,T?

summary(gam_ssplines_inter)
(r_2_squared <- summary(gam_ssplines_inter)$r.sq)
summary(gam_ssplines_inter)$s.pv
...
###############################################################################.
#### SPACE TIME MODEL WITH A RECTANGULAR MESH ####
###############################################################################.
...
# create the mesh
mesh_1 <- create.mesh.2D(p, order = 1, segments = isboundary)
mesh <- refine.mesh.2D(mesh_1, maximum_area=1.0, minimum_angle = 30)
basisobj <- create.FEM.basis(mesh)
