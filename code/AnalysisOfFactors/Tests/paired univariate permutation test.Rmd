...
GOAL:understand if the mean loss of colony during summer is significantly different from the one in the winter being, for each year, Y(i_summer)∼iid Y(summer) and Y(i_winter)∼iid Y(winter), we want to test the equality of the two distributions

H0:Y(summer)=Y(winter) vs H1:Y(summer)≠Y(winter) to avoid dependency, perform the test for each year.
